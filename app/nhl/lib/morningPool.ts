...
import { exportTop5 } from "./exportTop5";

const filePath = path.join(process.cwd(), "app/nhl/data/morningPool.json");
...
function readMorningPoolFile(): Record<string, any[]> {
  try {
    ...
    console.log("❌ Error leyendo archivo NHL morningPool:", err);
    return {};
  }
}
...
export function readMorningPool(dateKey?: string): any[] {
  try {
    ...
    console.log("❌ Error leyendo NHL morningPool:", err);
    return [];
  }
}
...
export function saveMorningPool(picks: any[], dateKey?: string) {
  try {
    ...
    exportTop5(allData);
    console.log(`✅ NHL Morning Pool guardado para ${targetDate}`);
  } catch (err) {
    console.log("❌ Error guardando NHL morningPool:", err);
  }
}