...
type MorningPoolData = Record<string, NhlMorningPick[]>;

function getTeams(pick: NhlMorningPick) {
...
function formatNhlPick(pick: NhlMorningPick) {
...
function getOdds(pick: NhlMorningPick) {
  return pick.originalPrice ?? pick.livePrice ?? pick.odd?.price ?? null;
}

function getLine(pick: NhlMorningPick) {
  return pick.originalPoint ?? pick.livePoint ?? pick.odd?.point ?? null;
}

function getScore(pick: NhlMorningPick) {
  if (typeof pick.score === "number") return pick.score;

  const confidence = pick.confidence ?? null;
  const valuePriority = pick.valuePriority ?? null;

  if (confidence !== null && valuePriority !== null) {
    return confidence * 0.45 + valuePriority * 0.55;
  }
...
export function exportTop5(morningData: MorningPoolData) {
  ...
  if (!dateKey) {
    console.log("No date found in NHL morningPool data");
    return;
  }
  ...
  const filePath = path.join(exportsDir, "nhl-top5.json");
  ...
  console.log("nhl-top5.json generated successfully");
}